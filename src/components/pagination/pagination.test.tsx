...
describe('Pagination', () => {
  ...
  it('renders first and last controls when enabled', () => {
    render(<Pagination page={3} pageCount={5} showFirstLast onPageChange={() => {}} />);
  ...
  it('renders ellipses for large ranges', () => {
    render(<Pagination page={50} pageCount={100} onPageChange={() => {}} />);

    const navigation = screen.getByRole('navigation', {
      name: 'Pagination',
    });

    expect(navigation.querySelectorAll('[data-slot="pagination-ellipsis"]')).toHaveLength(2);
  });
  ...
  it('does not render ellipses for small ranges', () => {
    ...
    expect(
      screen.getByRole('navigation').querySelector('[data-slot="pagination-ellipsis"]'),
    ).not.toBeInTheDocument();
  });

  it('renders nothing when pageCount is zero', () => {
    const { container } = render(<Pagination page={1} pageCount={0} onPageChange={() => {}} />);
  ...
  it('forwards native navigation props', () => {
    ...
    expect(navigation).toHaveClass('rush-pagination', 'custom-pagination');

    expect(navigation).toHaveAttribute('title', 'Record pagination');
  });
});
